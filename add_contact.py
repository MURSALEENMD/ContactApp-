...
def form():
 gui.load()
 Label(gui.container, text="Add New Contact") \
 ...
 btn_reset = Button(gui.action_frame, text="Reset", command=lambda: clear(), relief=GROOVE,
 bg='#dcdcdc')
 btn_reset.grid(row=0, column=2, pady=10, padx=10)
#save() of save button
def save():
 ...
 gui.text_phone.get(), gui.text_address.get())
 conn = db_connect()
 with conn:
    cur = conn.cursor()
 sql_query = "INSERT INTO tbl_contactlist (name, email, phone, address)"\
 "VALUES (?, ?, ?, ?)"
